...
이 저장소는 `main` 브랜치에 push되면 `.github/workflows/deploy-pages.yml`이 정적 웹 뷰를 빌드해서 GitHub Pages에 배포합니다.
...
현재 웹 뷰는 React, Vite, TypeScript 기반의 빈 문서 캔버스입니다. PolarisDesign 토큰 CSS를 사용하며, 앞으로 화면이 추가되면 같은 Pages workflow로 배포됩니다.

## Polaris UI 작업 규칙

모든 UI 작업은 [AGENTS.md](AGENTS.md)의 `Polaris UI Brief` 출력 규칙을 먼저 따릅니다.

- 톤앤매너 기준: [Polaris UI Contract](docs/design/polaris-ui-contract.md)
- Figma 산출물 기준: [Figma Output Contract](docs/design/figma-output-contract.md)
- 브리프 템플릿: [UI Brief Template](docs/design/ui-brief-template.md)
